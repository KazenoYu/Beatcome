...
function shopListInit(){
  changeAtagHrefWithCartListIdx(); //- 初始載入 戴上query
  renderCartDataFromUrl();  //- 依照 url.query list 渲染 購物車
  renderShopList();
}

function renderCartDataFromUrl(){ //- 依照 url.query 渲染 購物車
    let myList =  getParameterByName("list",window.location.url);
    if(myList){ //- url query有購物車資訊
        console.log("AA---1");
...
function changeAtagHrefWithCartListIdx(){ //- <a class="bind_query_A"> 的href query戴上 購物車資訊
    if(myShopListIndex.length > 0){ //- <a> tag跳轉href帶 購物車目前array index 參數
...
function getParameterByName(name, url) { //- 取得URL Query
...
function musicItemHtmlParse(data,nb){ //- 購物車單筆Item Element
...
function renderShopList(){ //- render購物車Lists element
  $('.musicLists').empty(); //-清空
  changeAtagHrefWithCartListIdx(); //- <a class="bind_query_A"> 的href query戴上 購物車資訊
  let totalPrice = 0;
  myShopList.forEach((ee, nb)=>{ //- 每一項重新渲染
...
$("#toggleShopList").on('click',function(){ //- 開闔shopCart
    $('#shoppingCart').toggleClass('show');
});

$( "img[alt='shopping-cart']").bind("click", function() { //- (Store.html)加入到購物車
    ...
    const isAlreadyInShop = myShopList.filter(ee=>ee.name == fileListsVocal[FoundIndex].name).length > 0; //- 是否已在購物車中
    if(!isAlreadyInShop){ //- 目標沒有在購物車中
...
function removeItem(nb){ //- 移除購物車Item
  nb=parseInt(nb);  
...
function playShopCartItem(nb){ //- 購物車撥放音樂
